  ...
  , angular_velocity_( 0 )
{
  ...
  topic_layout->addWidget( new QLabel( "Algorithm" ));
  output_topic_editor_ = new QLineEdit;
  topic_layout->addWidget( output_topic_editor_ );

  // 创建一个输入线速度的窗口
  topic_layout->addWidget( new QLabel( "Social Distance" ));
  output_topic_editor_1 = new QLineEdit;
  // output_topic_editor_1->setText("5");
  topic_layout->addWidget( output_topic_editor_1 );

  // 创建一个输入角速度的窗口
  topic_layout->addWidget( new QLabel( "Minimum Group Number" ));
  output_topic_editor_2 = new QLineEdit;
  // output_topic_editor_2->setText("5");
...
// 更新线速度值
void Group::update_Linear_Velocity()
{
    // 获取输入框内的数据
...
// 更新角速度值
void Group::update_Angular_Velocity()
{
...
// 更新topic命名
void Group::updateTopic()
{
  setTopic( output_topic_editor_->text() );
}

// 设置topic命名
void Group::setTopic( const QString& new_topic )
{
  // 检查topic是否发生改变.
  if( new_topic != output_topic_ )
  {
    ...
    else
    {
      ...
      velocity_publisher_ = nh_.advertise<my_msg::group>( output_topic_.toStdString(), 1 );
    }
...
void Group::sendVel()
{
  ...
  {
    my_msg::group msg;
    msg.eps = linear_velocity_;
    msg.minpts = angular_velocity_;
...
// 重载父类的功能
void Group::save( rviz::Config config ) const
{
...
// 重载父类的功能，加载配置数据
void Group::load( const rviz::Config& config )
{
...
PLUGINLIB_EXPORT_CLASS(rviz1::Group,rviz::Panel )
// END