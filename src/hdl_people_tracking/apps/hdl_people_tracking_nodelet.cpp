...
class HdlPeopleTrackingNodelet : public nodelet::Nodelet {
   ...
   private:
    ...
    visualization_msgs::MarkerArrayConstPtr create_tracked_people_marker(const std_msgs::Header& header) const {
        ...
        // boxes.type = visualization_msgs::Marker::LINE_STRIP;
        ...
        for (int i = 0; i < tracker->people.size(); i++) {
            ...
            // text.pose.position.x = round(text.pose.position.x*100)/100;
            // text.pose.position.y = round(text.pose.position.y*100)/100;
            ...
            float x = round(text.pose.position.x*10)/10;
            float y = round(text.pose.position.y*10)/10;
            text.text = (boost::format("id:%d (%d,%d)") % person->id() % x % y).str();
