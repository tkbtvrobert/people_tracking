...
class HdlPeopleDetectionNodelet : public nodelet::Nodelet {
   ...
   private:
    ...
    /**
     * @brief in case the sensor is fixed 如果傳感器是固定的
     * @param points_msg
     */
    ...
    void callback_static(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
        ...
        // background subtraction and people detection 背景減法和人物檢測
        auto filtered = backsub->filter(cloud);
        auto clusters = detector->detect(filtered);

        // if(print_fps_)if(++frames>10){NODELET_INFO("[CCU]: fps = %f, timestamp = %u", float(frames)/(float(clock()-start_time)/CLOCKS_PER_SEC), clock()/CLOCKS_PER_SEC);reset = true;}//fps
        // if(print_fps_)if(++frames>10){std::cerr<<"[CCU]: fps = "<<float(frames)/(float(clock()-start_time)/CLOCKS_PER_SEC)<<", timestamp = "<<clock()/CLOCKS_PER_SEC<<std::endl;reset = true;}
