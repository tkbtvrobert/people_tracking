...
def callback(human_msg, DBSCAN_msg, marker_array_msg):
    ...
    # print("Number of danger groups: ", len(group_num)-1)
    danger_table=[]
    ...
    # else:
    #     chart_msg.riskFactor = float(danger_sum) / float(len(human_msg.tracks))
    # rospy.loginfo (human_msg.tracks)
    # rospy.loginfo (float(len(human_msg.tracks)))
    ...
    for i in range(len(marker_array_msg.markers)):
        ...
        # mesh_msg.type = 10
        # # mesh_msg.pose.position.x = human_msg.tracks[i].pos.x - 0.256 # +0.256
        # mesh_msg.pose.position.x = marker_array_msg.markers[i].pose.position.x - 0.256
        # # mesh_msg.pose.position.y = human_msg.tracks[i].pos.y + 0.456 # -0.1
        # mesh_msg.pose.position.y = marker_array_msg.markers[i].pose.position.y + 0.456
        ...
        # mesh_msg.scale.x = 0.25 # 0.15
        # mesh_msg.scale.y = 0.25 # 0.15
        # mesh_msg.scale.z = 0.25 # 0.15
    ...
    if len(group_num) >= 2:
        ...
        write_csv(len(human_msg.tracks), len(group_num)-1, '{:.2%}'.format(chart_msg.riskFactor))
...
if __name__=="__main__":
    ...
    # table_pub = rospy.Publisher("danger_table", kdangerArray, queue_size=1) # Publish the danger table to rviz
