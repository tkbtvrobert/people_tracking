...
#include <kkl/alg/global_nearest_neighbor_association.hpp>
...
/**
 * @brief definition of the distance between tracker and observation for data association 為數據關聯定義跟踪器和觀察點之間的距離
 */
template<>
...
public:
  PeopleTracker(ros::NodeHandle& private_nh) {
    ...
    data_association.reset(new kkl::alg::GlobalNearestNeighborAssociation<KalmanTracker::Ptr, Cluster>());
  }

  /**
   * @brief predict people states 預測人們的狀態
...
public:
  long id_gen;                  // track ID which will be assigned to the next new track 將分配給下一個新軌道的軌道 ID
  double human_radius;          // new tracks must be far from existing tracks than this value 新軌道與現有軌道的距離必須大於此值
  double remove_trace_thresh;   // tracks with larger covariance trace than this will be removed 協方差軌跡大於此的軌跡將被刪除
