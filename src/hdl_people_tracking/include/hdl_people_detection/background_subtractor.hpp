 ...
 *        從另一個雲中減去點雲的類。 它使用稀疏矩陣創建佔用體素映射要減去的雲，然後刪除佔用體素中要減去的雲的每個點。
 */
...
public:
  ...
  /**
   * @brief set occupancy threshold 設置佔用閾值
   * @param t  voxels containing larger number of points than this value, are considered occupied 包含比此值更多的點的體素被視為已佔用
   */
  ...
  /**
   * @brief set background cloud 設置背景雲 (globalmap)
   * @param src_cloud
   */
  ...
  /**
   * @brief add points to occupancy voxel map 向佔用體素地圖添加點
   * @param bg_cloud
   */
  ...
  /**
   * @brief perform background subtraction 執行背景減法
   * @param src_cloud
  ...
  /**
   * @brief centroids of occupied voxels 佔用體素的質心
   * @return
   */
  ...
  /**
   * @brief create visualization marker of voxels 創建體素的可視化標記
   * @note  It seems rviz cannot deal with markers with very many primitives, so this method may not work well 看來 rviz 無法處理具有非常多基元的標記，因此這種方法可能效果不佳
   * @return
   */
